		...
		<div tw-dialog #dialog [(isOpened)]="opened">
			<div tw-dialog-container>
				...
				<div class="grid gap-3 text-center sm:text-left sm:flex sm:flex-row sm:items-start">
					<tw-icon name="exclamation-triangle" class="text-red-600 bg-red-100 dark:bg-opacity-50 dark:bg-red-900/50 rounded-full p-3 mx-auto" />
					<div class="text-pretty text-sm text-gray-600 dark:text-gray-400">
						...
						<p>
							Are you sure you want to deactivate your account? All of your data will be permanently removed from our servers forever. This action cannot be
							undone.
						</p>
						<!-- Dialog actions -->
						<div class="grid sm:flex gap-4 mt-5">
							<button tw-button (click)="dialog.close()" class="w-full sm:w-fit bg-red-600 hover:bg-red-600/90 active:bg-red-700/90 focus:outline-red-600">
								Deactivate
							</button>
