...
@Component({
	...
	template: `
		...
		>
			<button tw-button variant="secondary" (click)="dialog02.toggle()" [popup]="dialog02">View bottom-left notification dialog</button>
			<button tw-button variant="secondary" (click)="dialog03.toggle()" [popup]="dialog03">View bottom-right notification dialog</button>
		</div>
		<!-- bottom Left Dialog -->
		<div tw-dialog #dialog02 [isModal]="false" class="inset- bottom-0 left-0">
			...
			<div tw-dialog-container class="sm:max-w-sm">
				...
				<div class="grid gap-3 text-center sm:text-left">
					<h1 class="font-bold text-balance text-lg my-0 text-gray-700 dark:text-gray-300">Payment successful</h1>
					<p class="text-sm text-slate-600 dark:text-slate-400">
						Lorem ipsum dolor sit amet consectetur adipisicing elit. Consequatur amet labore. Lorem ipsum dolor sit amet consectetur adipisicing elit.
					</p>
				</div>
				<!-- Dialog actions -->
				<tw-icon (click)="dialog02.close()" source="heroicons" name="x-mark" class="absolute top-5 right-5 cursor-pointer" />
			</div>
		</div>
		...
		<div tw-dialog #dialog03 [isModal]="false" [autoClose]="true" class="inset- bottom-0 right-0">
			<!-- Dialog container -->
			<div tw-dialog-container class="sm:max-w-sm place-self-end opacity-100 not-in-open:translate-y-200 starting:translate-y-200 in-open:translate-y-0">
				<!-- Dialog content -->
				<div class="grid gap-3 text-center sm:text-left">
					<h1 class="font-bold text-balance text-lg my-0 text-gray-700 dark:text-gray-300">Payment successful</h1>
					<p class="text-sm text-slate-600 dark:text-slate-400">
				...
				<!-- Dialog actions -->
				<tw-icon (click)="dialog03.close()" source="heroicons" name="x-mark" class="absolute top-5 right-5 cursor-pointer" />
			</div>
		</div>
		<!-- Nested dialogs -->
	`,
