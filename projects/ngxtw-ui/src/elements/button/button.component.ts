...
import { ButtonToken, ComboboxToken, DialogToken, DropdownToken, PopupDirective, SizeOption } from '@ngxtw/core';
...
@Component({
  ...
  providers: [{ provide: ButtonToken, useExisting: ButtonComponent }]
})
export class ButtonComponent extends ButtonToken {
      ...
      write: () => {
        if (this.popup) {
          this.nativeElement.ariaHasPopup =
            this.popup instanceof DropdownToken ? 'menu' :
              this.popup instanceof ComboboxToken ? 'listbox' :
                this.popup instanceof DialogToken ? 'dialog' : 'true';
        }
      }
    })
  }
  ...
  protected onKeyDown(event: KeyboardEvent): void {
    ...
    if (this.popup && this.popup instanceof DropdownToken) {
